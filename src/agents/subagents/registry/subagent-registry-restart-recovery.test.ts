...
describe("subagent registry restart recovery", () => {
  ...
  it("retries an accepted recovery notice without holding the resumed run", async () => {
    ...
    await vi.waitFor(() => expect(markPendingNotice).toHaveBeenCalledOnce());
    ...
    expect(markPendingNotice).toHaveBeenCalledOnce();
