...
const markPendingNotice = vi.fn<RecoveryParams["markPendingNotice"]>((params) => {
  params.expected.resumptionNotice = { idempotencyKey: params.idempotencyKey };
  return true;
});
...
export const restartRecoveryTestHarness = {
  ...
  reset() {
    ...
    markPendingNotice.mockImplementation((params) => {
      params.expected.resumptionNotice = { idempotencyKey: params.idempotencyKey };
      return true;
    });
