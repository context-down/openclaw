...
export async function reconcileAcceptedRecovery(params: {
  ...
  markPendingNotice: RestartRecoveryParams["markPendingNotice"];
  now: number;
  ...
  if (noticeRequired) {
    try {
      if (
        !ownsAcceptedTarget() ||
        !params.markPendingNotice({
          runId: owner.runId,
          expected: owner,
          idempotencyKey: params.receipt.idempotencyKey,
        })
      ) {
        params.warn("accepted subagent restart recovery lost its notice-pending owner", {
          runId: owner.runId,
          childSessionKey: params.childSessionKey,
        });
        return { status: "deferred" };
      ...
      params.warn("accepted subagent restart recovery could not persist its notice debt", {
        runId: owner.runId,
        childSessionKey: params.childSessionKey,
        error,
      });
  ...
  if (resumptionConfirmed && noticeRequired) {
    try {
      if (
