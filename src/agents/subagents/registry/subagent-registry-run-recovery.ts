...
export class SubagentRecoveryManager extends SubagentWaitManager {
  ...
  readonly markPendingSubagentRecoveryNotice = (noticeParams: {
    runId: string;
    expected: SubagentRunRecord;
    idempotencyKey: string;
  }): boolean => {
    const runId = noticeParams.runId.trim();
    const idempotencyKey = noticeParams.idempotencyKey.trim();
    const entry = this.options.runs.get(runId);
    if (!runId || !idempotencyKey || entry !== noticeParams.expected) {
      return false;
    }
    ...
    entry.resumptionNotice = { idempotencyKey };
    try {
      this.options.persistOrThrow(runId);
    } catch (error) {
