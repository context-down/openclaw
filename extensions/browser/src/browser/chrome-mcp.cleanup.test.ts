...
describe.skipIf(process.platform === "win32")("Chrome MCP SDK-initiated cleanup", () => {
  it.each([false, true])(
    ...
    async (ephemeral) => {
      ...
      try {
        await expect(session.ready).rejects.toThrow("fixture initialization failed");
        ...
        expect(
          settled,
          "every close must join the SDK shutdown while its exact child is alive",
        ).toBe(0);
  ...
  it("retains failed initial capture after the child exits with an untracked descendant", async () => {
    ...
    try {
      ...
      await fixture.closed;
      releaseScan.resolve();
      await expect(fixture.session.ready).rejects.toThrow("fixture process census failed");
